...
async function retrieve(question) {
    ...
    // Large KB: semantic (hybrid) retrieval via Upstash Vector, keyword fallback.
    const index = getVectorIndex();
    if (!index) return retrieveKeyword(question);
    try {
        const matches = await index.query({ data: question, topK: 6, includeMetadata: true });
        const texts = (matches || [])
