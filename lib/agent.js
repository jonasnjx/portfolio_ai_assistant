...
async function buildApp() {
    ...
    async function nodeGenerate(state, config) {
        // gpt-oss-120b is a reasoning model: reasoning tokens count against maxTokens,
        // so keep effort low and leave headroom or the answer can come back empty.
        // Low temperature keeps the out-of-scope refusal guardrail reliable.
        const llm = new ChatGroq({ model: GEN_MODEL, temperature: 0.2, maxTokens: 500, maxRetries: 1, reasoningEffort: 'low' });
    ...
    async function nodeCritique(state, config) {
        ...
        const llm = new ChatGroq({ model: GEN_MODEL, temperature: 0, maxRetries: 1, reasoningEffort: 'low' });
